...
Future<(List<Map<String, dynamic>>, Object?, Object?, Object?, Object?)>
  ...
  final response = await http.get(Uri.parse('http://localhost:8081/v1/oa?page=0'));
...
Future<List<String>> fetchLevels(ref) async {
  ...
  if (response.statusCode == 200) {
    ...
    List<StudyLevelModel> studyLevels = jsonData
        .map((level) => StudyLevelModel(id: level["id"], nome: level["nome"]))
        .toList();
